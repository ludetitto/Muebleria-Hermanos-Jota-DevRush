...
const EditarProducto = () => {
        ...
        const fetchProducto = async () => {
            ...
            try {
                ...
                setFormData({
                    ...
                    imagen: data.imagen || "", categoria: data.categoria || "",
    ...
    const handleDetallesChange = (id, field, value) => {
        setDetallesList(
            detallesList.map((item) => (item.id === id ? { ...item, [field]: value } : item))
        );
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.nombre.trim() || formData.precio === "" || isNaN(Number(formData.precio))) {
            Swal.fire({ title: "Advertencia", text: "Debe completar el Nombre y un Precio válido.", icon: "warning" });
        ...
        const productoData = {
            ...
            precio: parseFloat(formData.precio), stock: parseInt(formData.stock, 10) || 0,
            imagen: formData.imagen.trim(), categoria: formData.categoria.trim(),
            destacado: formData.destacado, detalles: detallesObjeto,
        };
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || "Fallo en la actualización del producto.");
            }

            Swal.fire({ title: "¡Guardado!", text: `El producto '${productoData.nombre}' ha sido actualizado con éxito.`, icon: "success" });
            navigate("/productos");// Redirige catalogo

        } catch (error) {
            console.error("Error al actualizar el formulario:", error);
            Swal.fire({ title: "Error de Actualización", html: `No se pudo actualizar el producto.<br><b>Detalle:</b> ${error.message}`, icon: "error" });
        } finally {
            setIsSubmitting(false);
        }
    };
                                ...
                                <div className="column-left">
                                    <div className="form-card">
                                        ...
                                        <div className="form-group">
                                            ...
                                            <textarea id="descripcion" name="descripcion" value={formData.descripcion} onChange={handleChange} maxLength="1000" />
                                        </div>
                                        ...
                                        {detallesList.map((detalle) => (
                                            <div key={detalle.id} className="inline-group detail-item">
                                                <div className="form-group detail-key">
                                                    <input type="text" placeholder="Característica" value={detalle.key} onChange={(e) => handleDetallesChange(detalle.id, "key", e.target.value)} />
                                                </div>
                                                <div className="form-group detail-value">
                                                    <input type="text" placeholder="Valor" value={detalle.value} onChange={(e) => handleDetallesChange(detalle.id, "value", e.target.value)} />
                                                </div>
                                                {detallesList.length > 1 && (
                                                    <button type="button" className="btn-remove" onClick={() => handleRemoveDetalle(detalle.id)}>🗑️</button>
                                                )}
                                            </div>
                                        ))}
                                        <button type="button" className="btn-secondary add-detail-btn" onClick={handleAddDetalle}>+ Añadir Característica</button>
                                    </div>
                                </div>
                                ...
                                <div className="column-right">
                                    ...
                                    <div className="form-card">
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input type="text" id="imagen" name="imagen" value={formData.imagen} onChange={handleChange} placeholder="https:// o /uploads/img.jpg" />
                                        </div>
                                        {formData.imagen ? (
